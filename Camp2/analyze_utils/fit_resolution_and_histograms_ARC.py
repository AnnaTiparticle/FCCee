...
def fit_gaussian(values, method="original", nbins=80):
    ...
    for the reported fit, plus histogram data and chi2/ndof for diagnostics.
    """
    ...
    # final histogram at the window actually used, for plotting + chi2
    counts, edges = np.histogram(values, bins=nbins, range=(lo, hi))
    centers = 0.5 * (edges[:-1] + edges[1:])
    ...
    return dict(mu=mu, sigma=sigma, sigma_err=sigma_err, lo=lo, hi=hi,
                A_fit=A_plot, counts=counts, edges=edges, centers=centers,
                chi2=chi2, ndof=ndof)
...
# ── measurement ───────────────────────────────────────────────────────────────
def measure_resolution(fname, E_true, mode="analog", fit_method="original"):
    f    = uproot.open(fname)
    tree = f["events"]
    tree_keys = tree.keys()
...
for label, files in CONFIGS.items():
    for E, fname in zip(ENERGIES, files):
        if not fname or not os.path.exists(fname):
            print(f"  [skip] {label} {E}GeV: file not found ({fname})")
            continue
        r = measure_resolution(fname, float(E), mode=args.mode, fit_method=args.fit_method)
    ...
    for row, label in enumerate(labels_with_data):
        for col, E in enumerate(ENERGIES):
            ...
            ax.bar(centers, counts, width=(edges[1] - edges[0]),
                   color="lightgray", edgecolor="gray", linewidth=0.3)
            if sigma > 0:
                x_fine = np.linspace(lo, hi, 300)
                ax.plot(x_fine, gaussian(x_fine, A_fit, mu, sigma),
                        color="red", lw=1.3)
            ax.axvline(lo, color="blue", ls="--", lw=1)
            ax.axvline(hi, color="blue", ls="--", lw=1)

            chi2_txt = f"chi2/ndof={chi2:.1f}/{ndof}" if np.isfinite(chi2) else "chi2/ndof=n/a"
